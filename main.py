    ...
    if(size > TG_SPLIT_SIZE):
        ...
        try:
            ...
            if (mode and mode == "doc") or not mode:
                temp_file = app.send_document(message.chat.id, caption=f"{file}", thumb=thumb, document=file, reply_to_message_id=message.id, progress=progress, progress_args=[message])
            elif mode and mode == "video":
                temp_file = app.send_video(message.chat.id,caption=f"{file}" ,thumb=thumb, video=file, reply_to_message_id=message.id, progress=progress, progress_args=[message])
